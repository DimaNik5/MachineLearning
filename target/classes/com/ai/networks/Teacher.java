...
public interface Teacher<IN, OUT> {
    ...
    /**
     * setData - метод, позволяющий загрузить новые значения выборки
     * <p>
    ...
    /**
     * addData - метод, позволяющий добавить значения выборки
     * <p>
     * Примечание: размер входной выборки и выборки с ожидаемыми значениями должны совпадать и не равны 0.
     * </p>
     * @param inputs двумерный массив {@code IN}
    ...
    /**
     * addData - метод, позволяющий добавить одно значение выборки
    ...
    /**
     * getIn используется перед обучением для получения входных значений выборки
     * @return список массивов входных значений
     */
    List<IN[]> getIn();

    /**
     * getOut используется перед обучением для получения ожидаемых значений в выборке
     * @return список массивов выходных значений
     */
    List<OUT[]> getOut();

    /**
     * checkData используется перед {@code getIn} и {@code getOut}
