...
/**
 * Класс Matrix хранит матрицу (изображения) - двумерный массив {@code double}.
...
public class Matrix {
    ...
    public static final byte[] RGB_CHANNELS = {RED_MASK, GREEN_MASK, BLUE_MASK};
    public static final byte[] ALL_CHANNELS = {RED_MASK | GREEN_MASK | BLUE_MASK};
    ...
    /**
     * Данный конструктор позволяет извлекать матрицу из массива с определенного индекса
     * @param mat массив, содержащий матрицы
     * @param j индекс, с которого начинается матрица
    ...
    public Matrix(double[] mat, int j, int height, int width){
        ...
        for (int i = 0; i < height * width; i++) {
            matrix[i / width][i % width] = mat[i + j];
        }
    }

    /**
     * Метод для получения массива одноразмерных матриц из массива
     * @param mat массив, содержащий матрицы
     * @param count количество матриц
     * @param height высота матриц
     * @param width ширина матриц
     * @return массив извлеченных матриц
     */
